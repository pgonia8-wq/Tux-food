...
import { Store, Clock, Phone, MapPin, LogOut, ChevronRight, Image as ImageIcon, Settings } from 'lucide-react-native';
...
export default function RestauranteProfileScreen() {
  const { colors } = useTheme();
  const { user, logout } = useAuthStore();

  const menuItems = [
    { icon: Store, label: 'Datos del restaurante', color: colors.roleRestaurante },
    { icon: Clock, label: 'Horario de atención', color: colors.info },
    { icon: ImageIcon, label: 'Fotos y logo', color: colors.warning },
    { icon: MapPin, label: 'Dirección', color: colors.success },
    { icon: Settings, label: 'Configuración', color: colors.textSecondary },
  ];

  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <LinearGradient colors={[colors.roleRestaurante, colors.roleRestaurante + 'CC']} style={styles.header}>
